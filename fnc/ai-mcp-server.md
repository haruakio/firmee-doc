...
Claudeであれば設定画面からワンクリックでAI連携機能をONにします。オプトイン方式のため、会員が明示的にONにしない限り、データは一切AIサービスに送信されません。各サービスの連携方法の詳細はこちらをご参照ください。
...
* **メール起案**：「田中さんの事件の最新の電話メモを踏まえて返信メールを起案して」→ メモを参照しメール文面を生成
* **報告書の作成**：「メモを参考に昨日の期日の報告書を作成して」→ 期日メモから報告書を自動作成
* **請求書の作成**：「XX事件の請求書を作成して」→ 報酬・経費データから印影付きの請求書を生成
* **データ分析**：「2025年度の民事事件と刑事事件の売上を一覧表と円グラフにして」→ 売上データを集計し表とグラフを作成
